      ...
      <div ref="chatContainer" class="agent-message-area">
        ...
        >
          <div :class="msg.role === 'user' ? 'ios-chat-bubble-user' : 'ios-chat-bubble-agent'">
            <p v-if="msg.role === 'user'" class="whitespace-pre-wrap text-[15px] leading-relaxed">{{ msg.content }}</p>
      ...
      <div class="agent-composer">
        ...
        <div class="agent-composer-footer">
          <div class="agent-composer-left">
            ...
            <button 
              type="button"
              class="agent-web-search-btn"
              :class="{ 'agent-web-search-btn-active': webSearchEnabled }"
              @click="webSearchEnabled = !webSearchEnabled"
              :disabled="isLoading"
              :title="webSearchEnabled ? '已启用联网搜索' : '点击启用联网搜索'"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
              </svg>
              <span>{{ webSearchEnabled ? '联网搜索 开' : '联网搜索' }}</span>
            </button>
          </div>
