...
def memory_manager(state: KnowledgeAgentState) -> dict:
    ...
    # ── 0. checkpoint 感知：已有 history_messages 则跳过 ──
    existing_history = state.get("history_messages", [])
    if existing_history:
        logger.info(
            f"[MemoryManager] 从 checkpoint 恢复，跳过加载: "
            f"session={session_id}, 缓冲区={len(existing_history)}条, "
            f"history_prompt={len(state.get('history_prompt', '') or '')}字"
        )
        return {}  # 不修改任何 state，checkpoint 数据直接沿用
