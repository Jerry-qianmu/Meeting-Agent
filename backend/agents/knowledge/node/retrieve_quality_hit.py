...
def _maybe_rewrite_query(
    ...
    根据重试策略决定是否改写 query
    当前简化版本：直接返回原 query
    未来可以集成 query_rewrite 节点进行智能改写
    """
    # TODO: 可以调用 LLM 根据 retry_strategy 改写 query
    # 例如：
    # - broaden_query: 添加同义词、扩展关键词
    # - narrow_query: 添加限定词、缩小范围
    # - switch_retriever: 调整关键词权重
...
def _ragconfig_to_dict(config: RAGconfig) -> Dict[str, Any]:
    ...
    return {
        ...
        "group_by_field": getattr(config, "group_by_field", "doc_id"),
        "group_size": getattr(config, "group_size", 1),
        ...
        "max_context_tokens": getattr(config, "max_context_tokens", 1024),
        # quality control
