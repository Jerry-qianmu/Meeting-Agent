...
def query_rewrite(state: KnowledgeAgentState) -> dict:
    ...
    try:
        ...
        if not settings.llm_api_key:
            logger.warning("[QueryRewrite] API Key 未设置，使用原始 query")
            rewritten_query = original_query
        else:
            from service.llm_client import llm_call
            cfg = settings.get_llm_config("rewrite", model=state["config"].get("rewrite_model", settings.rewrite_model))
            result = llm_call(
                messages=messages,
                model=cfg["model"], api_type=cfg["api_type"], api_key=cfg["api_key"], base_url=cfg["base_url"],
            )
            if result["status_code"] == 200:
                rewritten_query = (result["content"] or "").strip()
            else:
                logger.warning(
                    f"[QueryRewrite] LLM 调用失败: status={result.get('status_code')}, 使用原始 query"
                )
                rewritten_query = ""
