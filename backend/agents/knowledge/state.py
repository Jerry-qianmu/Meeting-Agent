...
class KnowledgeAgentState(TypedDict, total=False):
    ...
    user_id: Optional[str]                              # 用户 ID（前端传入，记忆系统依赖）
    ...
    expanded_queries: Optional[List[str]]   # query expansion 生成的多查询变体
    sub_queries: Optional[List[str]]        # query decomposition 生成的子查询
    ...
    # =========================================================
    # 6.5 Chunk Merge Layer
    # =========================================================
    chunk_merged_chunks: List[Chunk]
    ...
    # 12. MCP Tool Calling
    # =========================================================
    available_tools: List[dict]                       # 可用工具 schemas（传给 LLM 的 tools 参数）
    tool_call_count: int                               # 当前轮工具调用次数（防止死循环）
    max_tool_calls: int                                # 最大工具调用次数（默认 5）
    web_search_enabled: bool                          # 用户是否启用联网搜索（前端开关）

    # =========================================================
    # 13. Memory Constellations 记忆系统
    # =========================================================
    memory_context: Optional[str]                        # 注入的记忆上下文（认知画像 + 相关碎片）
