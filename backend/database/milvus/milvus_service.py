...
class MilvusService:
    ...
    def hybrid_search(
        ...
        Args:
            search_limit: Milvus 内部 dense/BM25 各自的候选数量。
                          None 时使用 top_k（向后兼容）。
                          设大可以提高 RRF 融合质量，但会增加延迟。
        """
        ...
        # 内部搜索候选数 >= 最终返回数
        candidate_limit = search_limit if search_limit and search_limit > top_k else top_k
        ...
        # === dense 检索 ===
        # HNSW 要求 ef >= k(limit)，动态设置
        ef = max(100, candidate_limit)
        ...
        # 截断到 top_k（当 search_limit > top_k 时需要）
        if len(hits) > top_k:
            hits = hits[:top_k]

        mode = "keyword_filter+hybrid" if keyword_filter else "hybrid"
        group_info = f", group_by={group_by_field}, group_size={group_size}" if group_by_field else ", 无分组"
        logger.info(f"[Milvus] {mode} 返回 {len(hits)} 条 (collection={collection_name}, candidate_limit={candidate_limit}{group_info})")
    ...
    # ── Description Collection 已移除（直接检索 chunk）───────────────────────
    ...
    def get_chunks_by_ids(
        ...
        """根据 chunk_id 列表批量获取完整 chunk"""
        if not chunk_ids:
            return []

        fields = output_fields or ["chunk_id", "doc_id", "content", "chunk_index"]
        try:
            ...
            logger.error(f"[Milvus] 批量获取 chunk 失败: {e}")
            return []
