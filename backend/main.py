...
# 设置 DashScope 全局 API URL（MaaS 部署需要）
import dashscope
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    try:
        ...
        # 3.5 执行数据库迁移（幂等，重复执行自动跳过已存在的列）
        from database.mysql.migration_chunk_enrichment import run_migration
        ...
        # 6. 启动 Memory Constellations Archivist 后台任务
        try:
            from service.memory.archivist import InterviewArchivist
            archivist = InterviewArchivist(db_client=db_client)
            asyncio.create_task(archivist.start_tick_loop(interval_seconds=120))
            logger.info("Memory Archivist 后台任务已启动")
        except Exception as e:
            logger.warning(f"Memory Archivist 启动失败（不影响主功能）: {e}")
